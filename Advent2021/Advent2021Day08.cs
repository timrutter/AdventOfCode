...
namespace AdventOfCode.Advent2021
{
    public class Advent2021Day08 : Solution
    {
        ...
        public override object ExecutePart1()
        {
            ...
            var outputs = signals.Select(s => s.value).SelectMany(v => v.Split(" ", StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries)).ToList();
        ...
        public override object ExecutePart2()
        {
            var signals = DataFile.ReadAllKeyValuePairs<string, string>("|");
            var outputSum = 0;
            foreach ((string key, string value) in signals)
            {
                var codes = GetCodes(key.Split(" ", StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries).ToList());
