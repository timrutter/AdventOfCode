    ...
    public class Advent2021Day11 : Solution
    {
        ...
        public override object ExecutePart1()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                {
                    octopuses.SetValueAt(octopus.pos, octopus.value + 1);
                }
                ...
                while (!allDone)
                {
                    ...
                    foreach (var octopus in octopuses.ValuesAndPositions)
                    {
                        if (ProcessFlash(octopuses, octopus, flashed)) allDone = false;
                    }
                }
        ...
        private bool ProcessFlash(Board<int> octopuses, (Point pos, int value) octopus, List<Point> flashed)
        {
            ...
            if (!flashed.Contains(newPos) && octopuses.TryGetValueAt(newPos, out int val))
            {
                ret = true;
