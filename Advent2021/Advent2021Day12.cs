...
namespace AdventOfCode.Advent2021
{
    public class Advent2021Day12 : Solution
    {
        ...
        public override object ExecutePart1()
        {
            ...
            var routes = FindRoutes(lines, new List<string> { "start" });
            return routes.Count;
        }

        private static List<List<string>> FindRoutes(List<string[]> connections,  List<string> route)
        {
            if (route.Last() == "end") return new List<List<string>>{ route };
            var routes = new List<List<string>>( );
            var links = connections.Select(c => c[0] == route.Last( ) ? c[1] : c[1] == route.Last() ? c[0] : null).Where(c => c != null).ToList();
            var newRoutes = new List<List<string>>( );
            foreach (string link in links.Where(link => !route.Contains(link) || !char.IsLower(link[0])))
            {
        ...
        {
            var lines = DataFile.ReadAll<string>().Select(l => l.Split("-")).ToList();
            var routes = FindRoutes2(lines, new List<string> { "start" },"");
            return routes.Count;
        }
        private static List<List<string>> FindRoutes2(List<string[]> connections, List<string> route, string smallCave)
        {
            ...
            foreach (var newRoute in newRoutes) routes.AddRange(FindRoutes2(connections, newRoute.route, newRoute.smallCave ));
            return routes;
        }
    }
}