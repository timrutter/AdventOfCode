...
public abstract class Tree<TTree> where TTree : Tree<TTree>
{
    ...
    public void AddChildren(IEnumerable<TTree> children)
    {
        ...
        foreach (var child in enumerable) child.Parent = this as TTree;
...
{
    private readonly List<THeirarchy> _children;
    ...
    ///     The parent.
    /// </value>
    public List<THeirarchy> Parents { get; } = [];
    ...
    {
        var enumerable = children.ToList();
        foreach (var child in enumerable) child.Parents.Add(this as THeirarchy);
