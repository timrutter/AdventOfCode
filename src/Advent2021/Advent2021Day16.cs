...
public class Advent2021Day16 : Solution
{
    ...
    {
        return string.Concat(s.Select(c => Convert.ToString(Convert.ToInt32(c.ToString(), 16), 2).PadLeft(4, '0'))
            .ToList());
    }
    ...
    {
        var version = Convert.ToInt32(packet[..3], 2);
        ...
        if (packetTypeID == 4)
        {
            ...
            return (
                new Packet
                {
                    Version = version, PacketTypeID = packetTypeID,
                    Value = Convert.ToInt64(string.Concat(numberStrings), 2)
                }, i + 5);
        }
        ...
        if (packet[6] == '0')
        {
            ...
            while (read < length)
            {
                var (packet1, i) = ParsePacket(packet[(22 + read)..]);
                children.Add(packet1);
                read += i;
            }

            return (new Packet { Version = version, PacketTypeID = packetTypeID, Value = length, Children = children },
                22 + length);
        }
        ...
        if (packet[6] == '1')
        {
            ...
            {
                var (packet1, read1) = ParsePacket(packet[(18 + read)..]);
                children.Add(packet1);
                read += read1;
            }

            return (new Packet { Version = version, PacketTypeID = packetTypeID, Value = count, Children = children },
                18 + read);
        }
...
internal class Packet
{
    ...
    {
        return Version + Children.Sum(c => c.SumVersions());
    }
