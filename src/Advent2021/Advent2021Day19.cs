...
public class Advent2021Day19 : Solution
{
    ...
    private bool MatchTransformAndSetPosition(Scanner scanner, Scanner scanner2)
    {
        foreach (var t in _transformations)
        {
            ...
            for (var i = 0; i < scanner2.Beacons.Count; i++) scanner2.Beacons[i] += scannerPos;
            return true;
        }
    ...
    public class Translation
    {
        ...
        {
            var ret = point3D;
            foreach (var swapMode in _swapModes) ret = ApplySwapMode(ret, swapMode);
    ...
    public class Scanner
    {
        ...
        {
            for (var i = 0; i < Beacons.Count; i++) Beacons[i] = translation.Apply(Beacons[i]);
        }

        public Scanner Transform(Translation translation)
        {
            var ret = new Scanner();
            foreach (var point3D in Beacons) ret.Beacons.Add(translation.Apply(point3D));
